...
namespace InspectorGadgets.Editor.PropertyDrawers
{
    internal sealed class PositionDrawer : TransformPropertyDrawer
    {
        ...
        protected override Vector3 GetDragDirection(Transform target) => target.forward;
        ...
        public static readonly AutoPrefs.EditorInt
            SnapToGroundLayers = new AutoPrefs.EditorInt(Strings.PrefsKeyPrefix + "SnapToGroundLayers",
        ...
        public static readonly AutoPrefs.EditorBool
            ShowMovementGuides = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowMovementGuides), true),
            ShowMovementDistance = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowMovementDistance), true),
            ShowMovementDistancePerAxis = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowMovementDistancePerAxis), true),
            ShowPositionLabels = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowPositionLabels), false);
                        ...
                        if (MovingFromPositions.TryGetValue(target, out var movingFrom))
                        {
                            ...
                            if (ShowMovementDistance)
                            {
                                ...
                                if (distance > 0.00001f)
                                {
                                    ...
                                    {
                                        label = $"{label}" +
                                            $"\n<color=#{IGEditorUtils.ColorToHex(FieldColorX)}>{Math.Abs(offset.x)}</color>" +
                                            $"\n<color=#{IGEditorUtils.ColorToHex(FieldColorY)}>{Math.Abs(offset.y)}</color>" +
                                            $"\n<color=#{IGEditorUtils.ColorToHex(FieldColorZ)}>{Math.Abs(offset.z)}</color>";
