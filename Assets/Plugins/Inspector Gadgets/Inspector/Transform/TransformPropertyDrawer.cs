...
namespace InspectorGadgets.Editor.PropertyDrawers
{
    internal abstract class TransformPropertyDrawer
    {
        ...
        public static readonly AutoPrefs.EditorBool
            ShowCopyButton = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowCopyButton), true),
            ShowPasteButton = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowPasteButton), true),
            ShowSnapButton = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowSnapButton), true),
            ShowResetButton = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShowResetButton), false),
            DisableUselessButtons = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(DisableUselessButtons), true),
            UseFieldColors = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(UseFieldColors), true),
            EmphasizeNonDefaultFields = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(EmphasizeNonDefaultFields), true),
            ItaliciseNonSnappedFields = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ItaliciseNonSnappedFields), true),
            ShrinkScientificFields = new AutoPrefs.EditorBool(Strings.PrefsKeyPrefix + nameof(ShrinkScientificFields), true);
        ...
        public static readonly AutoPrefs.EditorFloat
            FieldPrimaryColor = new AutoPrefs.EditorFloat(Strings.PrefsKeyPrefix + nameof(FieldPrimaryColor),
                1, onValueChanged: (value) => GenerateFieldColors()),
            FieldSecondaryColor = new AutoPrefs.EditorFloat(Strings.PrefsKeyPrefix + nameof(FieldSecondaryColor),
            ...
            float? value, float snapValue, float defaultValue)
        {
            ...
            if (EmphasizeNonDefaultFields && (value == null || value.Value != defaultValue))
            {
                ...
                box.xMin += EditorGUIUtility.labelWidth - Border + EditorGUIUtility.standardVerticalSpacing;
                box.y -= Border;
                box.width += Border;
                box.height += Border * 2;
            ...
            {
                // If we didn't draw the box, get a control ID to make sure that the field's ID is consistent between
                // showing and not showing the box.
            ...
            if (!EditorGUI.showMixedValue)
            {
                ...
                // Small text for values using scientific notation.
                if (ShrinkScientificFields && IsScientific(value.Value))
                {
                    style.fontSize = (int)(0.75f * (fontSize > 0 ?
                        fontSize :
                        Height - style.margin.top - style.padding.vertical));
                }
            }

            // Draw the number field.
            fieldValue = EditorGUI.FloatField(area, label, fieldValue, style);
        ...
        private static Dictionary<float, bool> _ValueToIsScientific;

        private static bool IsScientific(float value)
        {
            if (value == 0 ||
                value == 1)
                return false;
            ...
            if (_ValueToIsScientific == null)
                _ValueToIsScientific = new Dictionary<float, bool>(Capacity);
            else if (_ValueToIsScientific.Count >= Capacity)
                _ValueToIsScientific.Clear();

            if (!_ValueToIsScientific.TryGetValue(value, out var result))
            {
                result = value.ToString().LastIndexOfAny(Ee) >= 0;
                _ValueToIsScientific.Add(value, result);
            }
        ...
        private void DoCopyButton(ref Rect area)
        {
            ...
            if (GUI.Button(buttonArea, Strings.GUI.Copy, InternalGUI.MiniSquareButtonStyle))
            {
        ...
        private void DoResetButton(ref Rect area)
        {
            ...
            if (GUI.Button(buttonArea, Strings.GUI.Reset, InternalGUI.MiniSquareButtonStyle))
            {
                GUIUtility.keyboardControl = 0;
